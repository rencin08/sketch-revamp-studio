...
import { Sparkles, ChevronLeft, Lightbulb, Plus } from "lucide-react";
...
const AIHub = () => {
  ...
  const handleSave = async () => {
    ...
    await addPin({ type: "note", content: writeContent.trim(), tags: ["idea"], archived: false, visibility: "private" });
    setWriteContent("");
    setIsWriting(false);
  };
          ...
          <motion.div key="pin-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="max-w-2xl mx-auto px-6 py-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-body text-xs tracking-[0.2em] text-accent uppercase">Idea</span>
              </div>
      ...
      <div className="max-w-3xl mx-auto px-6 pt-8 pb-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display italic text-3xl text-foreground">AI Hub</h1>
              <p className="font-body text-sm text-muted-foreground">Learning notes, ideas, and explorations.</p>
            </div>
          </div>
          {isOwner && !isWriting && (
      ...
      <div className="max-w-3xl mx-auto px-6 pb-12">
        {aiPins.length === 0 ? (
          <motion.div className="text-center py-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {isOwner && <button onClick={() => setIsWriting(true)} className="font-body text-sm text-accent hover:text-foreground transition-colors mt-2">add your first idea →</button>}
          </motion.div>
        ) : (
